...
export const Calendar: React.FC<CalendarProps> = ({
  ...
  const calendarDays = useMemo(() => {
    ...
    const endOfMonth = dateAdapter.endOfMonth(currentDate);
    ...
    while (dateAdapter.isBefore(current, endOfCalendar)) {
      ...
      if (isRangeSelection && selectedRange) {
        ...
        if (start && end) {
          isInRange =
            (dateAdapter.isAfter(current, start) || dateAdapter.isSameDay(current, start)) &&
            (dateAdapter.isBefore(current, end) || dateAdapter.isSameDay(current, end));
          ...
          isInRange =
            (dateAdapter.isAfter(current, rangeStart) || dateAdapter.isSameDay(current, rangeStart)) &&
            (dateAdapter.isBefore(current, rangeEnd) || dateAdapter.isSameDay(current, rangeEnd));
        }
      }
    ...
    <div className={calendarClassName}>
      ...
      <div className="rdl-calendar-grid">
        <div className="rdl-calendar-weekdays">
          {showWeekNumbers && <div className="rdl-calendar-weekday rdl-calendar-week-number-header">Wk</div>}
              ...
              {calendarDays.slice(weekIndex * 7, (weekIndex + 1) * 7).map((day, dayIndex) => {
                ...
                return (
                  <button
                    ...
                    aria-label={dateAdapter.format(day.date, locale.dateFormat)}
